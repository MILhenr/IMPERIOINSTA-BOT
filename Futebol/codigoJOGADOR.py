...
destino_pasta = os.path.expanduser('~/Desktop/ResumoJ')
...
numeros_usados = [
    ...
    if f.startswith('RJ') and f[1:-4].isdigit()
]
...
for video in sorted(videos_novos):
    while indice in numeros_usados:
        indice += 1
    novo_nome = f'RJ{indice}{os.path.splitext(video)[1]}'
...
# Pastas e caminhos
pasta_saida = os.path.join(destino_pasta, 'JogadorEditado')
os.makedirs(pasta_saida, exist_ok=True)

fundos = [
    os.path.join(destino_pasta, 'EXEMPLORJ1.MOV'),
    os.path.join(destino_pasta, 'EXEMPLORJ2.MOV')
]
...
videos_para_editar = sorted([
    ...
    if f.startswith('RJ') and f.lower().endswith(('.mp4', '.mov'))
])
...
for video_nome in videos_para_editar:
    ...
    if 'EXEMPLORJ1' in fundo_path:
        altura_resize = fundo.h * 0.64
        posicao_y = 1150
    elif 'EXEMPLORJ2' in fundo_path:
        altura_resize = fundo.h * 0.69
        posicao_y = 1180
    else:
