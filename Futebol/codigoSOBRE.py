...
destino_pasta = os.path.expanduser('~/Desktop/InstaSobre')
...
numeros_usados = [
    ...
    if f.startswith('SB') and f[1:-4].isdigit()
]
...
for video in sorted(videos_novos):
    while indice in numeros_usados:
        indice += 1
    novo_nome = f'SB{indice}{os.path.splitext(video)[1]}'
...
# Pastas e caminhos
pasta_saida = os.path.join(destino_pasta, 'SobreInsta')
os.makedirs(pasta_saida, exist_ok=True)

fundos = [
    os.path.join(destino_pasta, 'EXEMPLOsb2.MOV')
]
...
videos_para_editar = sorted([
    ...
    if f.startswith('SB') and f.lower().endswith(('.mp4', '.mov'))
])
...
print("✅ Todos os vídeos foram processados Sobre Insta!")