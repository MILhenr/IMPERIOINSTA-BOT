...
destino_pasta = os.path.expanduser('~/Desktop/AquiFutsal')
...
numeros_usados = [
    ...
    if f.startswith('AQ') and f[1:-4].isdigit()
]
...
for video in sorted(videos_novos):
    while indice in numeros_usados:
        indice += 1
    novo_nome = f'AQ{indice}{os.path.splitext(video)[1]}'
...
# Pastas e caminhos
pasta_saida = os.path.join(destino_pasta, 'AquiEditado')
os.makedirs(pasta_saida, exist_ok=True)

fundos = [
    os.path.join(destino_pasta, 'EXEMPLOA1.MOV'),
    os.path.join(destino_pasta, 'EXEMPLOA2.MOV')
]
...
videos_para_editar = sorted([
    ...
    if f.startswith('AQ') and f.lower().endswith(('.mp4', '.mov'))
])
...
for video_nome in videos_para_editar:
    ...
    if 'EXEMPLOA1' in fundo_path:
        altura_resize = fundo.h * 0.57
        posicao_y = 1270
    elif 'EXEMPLOA2' in fundo_path:
